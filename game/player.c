...
bool player_add_item_to_inventory(Player* player, Item item) {
    ...
    if (item.type == 1) { // Arma
        if (item.damage > player->equippedWeapon.damage) {
            printf("\033[1;32m¡Has encontrado un arma mejor! Equipada automaticamente.\033[0m\n");
        ...
        if (item.defense > player->equippedArmor.defense) {
            printf("\033[1;32m¡Has encontrado una armadura mejor! Equipada automaticamente.\033[0m\n");
...
void player_add_initial_class_items(Player* player, const char* initial_items_csv) {
    ...
    printf("\033[1;32m¡Has recibido tu equipo inicial, para la clase %s!\033[0m\n", player->name);
    printf("\033[1;36m----------------------------------------------\033[0m\n");
